...
export function SignupFormDemo() {
  ...
  const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    try{
    const res= await axios.post(process.env.NEXT_PUBLIC_BackendURL+"api/auth/signup" || "", inputs)
    ...
    <div className="shadow-input mx-auto w-full max-w-md rounded-none bg-white p-2 md:rounded-2xl md:p-8 dark:bg-black">
      ...
      <form className="my-8">
          ...
          <LabelInputContainer className="mb-4" >
            ...
            <Input id="Name" placeholder="Jay" type="text" onChange={(e)=>(setinputs({...inputs, name:e.target.value}))} />
          </LabelInputContainer>
        ...
        <LabelInputContainer className="mb-4">
          ...
          <Input id="email" placeholder="jaydeep@gmail.com" type="email"  onChange={(e)=>(setinputs({...inputs, email:e.target.value}))}/>
        </LabelInputContainer>
        <LabelInputContainer className="mb-4">
  ...
  <div className="flex-1 h-[1px] bg-gradient-to-r from-transparent via-neutral-300 to-transparent dark:via-neutral-700" />
        ...
        <div className="flex justify-center flex-row gap-2 ">
          <a
  href="https://backend-n70l.onrender.com/api/auth/google"
